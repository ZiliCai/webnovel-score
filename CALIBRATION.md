...
**范围**：开局级冒烟（样本为开局节选，非整本 map-reduce）；haiku 评分；generic 权重。
**对照**：网文铁爆款 vs 传统推理（后者作为「非网文市场向」的低契合代理——本地样本无真实扑街网文对照）。

| 书 | 总分 | 评级 | stance_clean | schema | 硬伤机制 |
|---|---|---|---|---|---|
| 斗破苍穹（爆款网文开局） | 88 | S | ✅ | 0 err | 节奏拖沓 |
| 无人生还（阿加莎·传统推理） | 42 | C | ✅ | 0 err | 开局劝退 / 钩子缺失 / 节奏拖沓 / 中段掉读 |

- **区分度 46 分**，`assert_calibration(margin=10)` 通过；方向正确（网文爆款 ≫ 非网文向）。
- **立场中立**：双方 stance_clean=True；无人生还硬伤全部归到商业机制，无任何男凝/价值观/道德类扣分。头号失败模式验证通过。
- **schema**：两份输出 0 错误，端到端管道跑通。

**尚未覆盖（真实发布前仍需补）**：
1. 整本 map-reduce 端到端（本次仅开局；全书逐章 map+聚合路径由 49 单测覆盖，但未在真实长篇上跑过）。
2. 同书重跑「方差 < 5」。
3. 真实扑街**网文**对照（当前用非网文代理，只能验方向、难标定中段刻度）。
