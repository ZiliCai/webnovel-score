...
## Stage 2 · 逐章信号 MAP（按上下文预算分块，每批一个独立子 agent）
这一步是 map，干的是**客观提取，不是打分**：抽每章的钩子 / 爽点 / 情绪 / 是否水章等**事实型、锚定**信号。真正的 6 维打分只在 Stage 4 一个 agent 手里（见下），所以这里的准则一致性靠「锚定档 + 同批同一个 agent」保证，不靠 90 个 agent 各判各的。

- **分块而非逐章**：把章节按**上下文预算**分成若干批（默认约 40-50k 字/批，装得进一个上下文即可），**每批派一个独立子 agent**——批与批之间上下文隔离、互不共享；每个 agent 一次读它那一批连续章节 + 注入立场声明的 `references/prompt-逐章信号.md`，一次性吐这批**每章一张**信号卡。**90 章的书 ≈ 几个 agent，不是 90 个。**
- **同批同尺**：一批内多章由同一个 agent 判，用的是同一把尺子；批越大跨章越一致。
- **批次剧情摘要**：每批子 agent 除信号卡外，附 2-4 句该批剧情摘要（主线推进 / 新设定 / 卖点变化）。这是 Stage 4 判「题材卖点 / 长线承载」的中后段依据——没有它评委只能凭开局猜。
- **并发**：各批彼此独立，**并发派发**；一轮回收后再派下一轮。
- **降级**：环境没有子任务工具、或已在子代理内 → 顺序分批（Effective Mode: sequential），语义一致、只是慢。
- 每张信号卡按 `schemas/signal_card.schema.json`、**bash 运行** `scripts/validate_output.py` 校验；不合规让该批子 agent 重出。
- **超长书**：批数随书长线性增长；实在过大可对**采样**章节深读、其余轻扫，但必须 `log()` 说明丢了哪些章（不静默截断）。
- 单批失败不阻断，记失败记录，最终可 completed_with_errors。

## Stage 3 · 曲线聚合 REDUCE（确定性 · bash）
**bash 运行** `scripts/aggregate_curves.py`，把所有信号卡聚合成钩子 / 爽点 / 情绪曲线 + 水章率 + 掉读风险章。纯脚本，不碰原文。

## Stage 3.5 · 抽查深读（一个独立子 agent · 中后段核验）
开局再好，中段崩了照样掉订阅。派**一个独立子 agent**：从开局之后的章节抽 2-3 章**全文**（**优先抽 Stage 3 标出的掉读风险章**，不足则按约 50% / 85% 位置取），连同这些章的信号卡 + 注入立场声明的 `references/prompt-抽查深读.md`。它核验信号卡是否属实、按「铺垫→释放→余波」判爽点执行质量、按商业口径判中后段成色。产出抽查报告，喂给 Stage 4 与 Stage 4.5。章数太少（≤开局章数+2）可跳过。

## Stage 4 · 维度评分 SYNTHESIS（一个独立子 agent · 全书唯一打分处）
**6 维分数只在这里产生，由这一个 agent 用一把尺子判**——所以跨章准则一致，不受 Stage 2 分批影响。派**一个独立子 agent**：它的上下文只有「开局卡 + Stage 3 曲线 + 批次剧情摘要 + Stage 3.5 抽查报告 + 可选摘要」（**不给原文全书**）+ 注入立场声明的 `references/prompt-维度评分.md`，对 6 维各给 0-100（按 `references/rubric.md` 四档 + `references/weights.json`；`rubric.md` 不可读 → 用其内置 embedded fallback，报 Rubric Source: embedded fallback）。产出按 `schemas/synthesis.schema.json` 校验。
...
派**一个独立子 agent**用 `references/prompt-反调.md` 抓过度乐观并二次道德 scrub——**把评分结果连同 Stage 3 曲线、Stage 3.5 抽查报告一并给它**，有证据才下调，不空对空；再 **bash 运行** `scripts/scan_moral_drift.py` 兜底删除漏网道德 finding（hard_issues + dimensions[].rationale）。
...
| references/prompt-开局深读.md · references/prompt-逐章信号.md · references/prompt-抽查深读.md · references/prompt-维度评分.md · references/prompt-反调.md | 各阶段 portable prompt（子 agent 用） |
